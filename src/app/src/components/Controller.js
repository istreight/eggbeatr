...
class Controller extends React.Component {
    ...
    init() {
        ...
        var initializations = [
            this.state.connector.getHeaderData(populate).then(
                (res) => this.setComponentData("header", res, false)
            )
            , this.state.connector.getGridData(populate).then(
                (res) => this.setComponentData("grid", res, false)
            )
            , this.state.connector.getInstructorData(populate).then(
                (res) => this.setComponentData("instructors", res
                    , false)
            )
            , this.state.connector.getLessonData(populate).then(
                (res) => this.setComponentData("lessons", res
                    , false)
            )
            , this.state.connector.getPreferenceData(populate).then(
                (res) => this.setComponentData(
                    "instructorPreferences", res, false)
            )
            , this.state.connector.getPrivatesData(populate).then(
                (res) => this.setComponentData("privates", res
                    , false)
            )
        ];
        return Promise.all(initializations).catch((error) => console.error(
            error));
    }
    /**
    ...
    renderComponents() {
        ...
        this.renderComponent(Landing, {}, document.getElementById(
            "dynamicLanding"));
        ...
        this.renderComponent(About, {}, document.getElementById(
            "dynamicAbout"));
        ...
        setGridChecklistQuantity = componentObjects.gridChecklist.setQuantity
            .bind(
                componentObjects.gridChecklist
            );
        this.renderComponent(
            Grid, {
                callback: componentCallback
                , createComponent: createComponent
                , getGrids: this.state.connector.getGridArrays.bind(
                    this)
                , gridChecklistCallback: componentObjects.gridChecklist
                    .setCreateGridComponents.bind(
                        componentObjects.gridChecklist
                    )
                , initData: components.grid
                , getSetTitle: componentObjects.header.getSelectedSet
                    .bind(
                        componentObjects.header
                    )
                , removeComponent: removeComponent
        ...
        this.renderComponent(InstructorPreferences, {
            ...
            "dynamicInstructorPreferences"), function() {
            setComponentData("instructorPreferences", this
                , true);
        });
        this.renderComponent(
            Instructors, {
                callback: componentCallback
                , createComponent: createComponent
                , deletePreference: componentObjects.instructorPreferences
                    .deletePreference.bind(
                        componentObjects.instructorPreferences
                    )
                , handlePreferencesClick: componentObjects.instructorPreferences
                    .displayComponentState.bind(
                        componentObjects.instructorPreferences
                    )
        ...
        this.state.connector.getReleaseVersion().then((res) => {
            var headerState = JSON.parse(JSON.stringify(
                components.header));
    ...
    componentCallback(component, componentName, updateDatabase) {
        ...
        console.log(componentName + ": ", this.state.components[
            componentName]);
        this.state.components[componentName] = JSON.parse(JSON.stringify(
            component));
        console.log(componentName + ": ", this.state.components[
            componentName]);
        console.log(
            "adding data from " + componentName +
            ".js to controllerData..."
        );
        if (componentName === "header") {
            this.state.connector.setHeaderId(component.data.selectedSet
                .id);
            return this.init().then(() => this.updateComponents(
                updateDatabase));
        } else {
        ...
        for (let name in this.state.componentObjects) {
            ...
            if (this.state.components[name] !== undefined) {
                ...
                if (name === "instructorPreferences") {
                    ...
                    instructorPreferencesComponent.state = this.state.components[
                        name];
                } else if (name === "grid") {
                    ...
                    instructorsComponent.state = this.state.components[
                        name];
                    ...
                    privatesComponent.privateLessons = this.state.components[
                        name];
                    this.state.componentObjects.gridChecklist.setQuantity(
                        "privates"
    ...
    manipulateData(componentName, updateDatabase) {
        ...
        this.state.controllerData.instructorsArray = Object.keys(
            instructors);
    ...
    updateDatabase(database) {
        ...
        if (database === "grid") {
            this.state.connector.updateGridData(
                this.state.components.grid.data.id
                , this.state.components.grid.data
            ).then((gridRes) => console.log("Updated Grid:"
                , gridRes));
            ...
            for (let key in instructorsData) {
                ...
                instructorUpdates.push(this.state.connector.updateInstructorData(
                    id, body));
            }
            Promise.all(instructorUpdates).then((instructorRes) => {
                this.state.components.instructors = this.assignUpdates(
                    instructorRes);
                console.log("Updated Instructors:", this.state.components
                    .instructors);
            });
            ...
            for (let key in lessonData) {
                ...
                lessonsUpdates.push(this.state.connector.updateLessonData(
                    id, body));
            }
            Promise.all(lessonsUpdates).then((lessonRes) => {
                this.state.components.lessons = this.assignUpdates(
                    lessonRes);
                console.log("Updated Lessons:", this.state.components
                    .lessons);
            });
        } else if (database === "instructorPreferences") {
            let preferenceUpdates = [];
            let preferenceData = this.state.components.instructorPreferences
                .data;
            for (let key in preferenceData) {
                ...
                preferenceUpdates.push(this.state.connector.updatePreferenceData(
                    id, body));
            }
            Promise.all(preferenceUpdates).then((preferenceRes) => {
                this.state.components.instructorPreferences =
                    this.assignUpdates(preferenceRes);
                console.log(
                    "Updated InstructorPreferences:"
                    , this.state.components.instructorPreferences
                );
            });
            ...
            for (let key in privatesData) {
                ...
                for (let i = 0; i < privateInstructor.length; i++) {
                    ...
                    privatesUpdates.push(this.state.connector.updatePrivatesData(
                        id, body));
                }
            }
            Promise.all(privatesUpdates).then((privatesRes) => {
                this.state.components.privates = this.assignUpdates(
                    privatesRes);
                console.log("Updated Privates:", this.state.components
                    .privates);
            });
        }
    ...
    removeComponent(id, component) {
        ...
        console.log(
            "Sent delete " + component + " request to database."
        );
        return promise;
    }
}

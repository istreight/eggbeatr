...
class Grid extends React.Component {
    ...
    init() {
        ...
        this.controllerData = this.props.callback(this.state, "grid"
            , false);
    }
    /**
    ...
    setDuration(newDuration) {
        ...
        if (!isDurationSet) {
            ...
            this.durationContainer[0].setState({
                styleClass: titleStyleClass.replace(
                    reReplaceCriteria, "")
            });
        }
    ...
    verifyData() {
        ...
        var validLessonTimes = lessonTimes.reduce(
            (result, current) => result && this.validateStartTime(
                current)
            , true
        );
        return validDuration && validLessonTimes && lessonTimes.length >
            0;
    }
    /**
    ...
    setLessonTimes(e) {
        ...
        if (e) {
            if (this.state && this.state.data && this.state.data.id) {
                ...
                this.props.createComponent(this.state.data, "Grid").then(
    ...
    noGridsNotification(
        ...
        , numInstructors
        , numHalfLessons
        , numThreeQuarterLessons
    ) {
        var noGridsString = "Add 0 Instructor";
        var instructorTime = duration * numInstructors;
        var lessonTime = 0.5 * numHalfLessons + 0.75 *
            numThreeQuarterLessons;
        var numAddInstructors = Math.floor((lessonTime - instructorTime) /
            duration) + 1;
        if (isNaN(numAddInstructors) || numAddInstructors < 1) {
            this.handleGridError();
            return;
        }
        var notification = noGridsString.replace(/[0-9]/
            , numAddInstructors);
        ...
        this.emptyGridsNotification.setState({
            ...
            , styleClass: styleClass.replace("is-invisible"
                , "is-visible")
        });
    ...
    getPreferenceLength(newInstructor) {
        var instructor = this.controllerData.instructorPreferences.data[
            newInstructor];
    ...
    orderInstructorsByPreferencesSize() {
        ...
        for (let instructorName in this.controllerData.instructorPreferences
                .data) {
            var preferenceSize = this.getPreferenceLength(
                instructorName);
        ...
        var noPreferenceInstructors = this.controllerData.instructorsArray
            .filter(
                (instructor) => {
                    return !newInstructorOrder.includes(instructor);
                }
            );
        var randomOrderInstructors = this.randomizeArray(
            noPreferenceInstructors);
    ...
    getPrivatesOnlyInstructors() {
        return this.controllerData.instructorsArray.filter((value) => {
            var instructor = this.controllerData.instructors.data[
                value];
    ...
    getGroupInstructors() {
        return this.controllerData.instructorsArray.filter((value) => {
            var instructor = this.controllerData.instructors[
                value];
            return !instructor.privatesOnly;
        });
    }
    ...
    getGridArrays(lessonQueue) {
        return this.props.getGrids(Object.assign(this.controllerData, {
            duration: this.state.data.duration
        ...
        for (let grid = 0; grid < newGrids.length; grid++) {
            ...
            for (let name = 0; name < instructorOrder.length; name++) {
                for (let slot = 1; slot < newGrids[grid][0].length; slot++) {
                    for (let instructor = 1; instructor < newGrids[grid]
                        .length; instructor++) {
                        let reactKey = "key-grid-" + name + instructor +
                            slot;
                        if (newGrids[grid][instructor][0] ===
                            instructorOrder[name]) {
                            this.assignLessons(newGrids[grid][
                                instructor
                            ], slot, queue, reactKey);
                            break;
                        }
    ...
    handleGridError() {
        this.waitIndicator.setState({
            data: [
                "ERROR", < p key = "key-grid-indicator-0" >
                An error occured creating grids. <
                br / >
    ...
    displayComponentState() {
        ...
        this.getGridArrays(lessonQueue).then(
            (gridArrays) => this.displayGrid(gridArrays
                , lessonQueue)
        ).catch((error) => {
            console.error(error);
            this.handleGridError();
    ...
    displayGrid(gridArrays, lessonQueue) {
        ...
        this.emptyGridsNotification.setState({
            styleClass: emptyStyleClass.replace("is-visible"
                , "is-invisible")
        });
    ...
    createTable(duration, gridArrays, gridIndex, lessonQueue) {
        ...
        var styleBody = {};
        for (
            let instructor = 0; instructor < gridArrays[gridIndex].length; instructor++
        ) {
            let newData = [];
            ...
            for (let slot = 0; slot < 2 * duration + 1; slot++) {
                ...
                if (Array.isArray(cellContents)) {
                    style = cellContents[cellContents.length - 1].props
                        .className;
                ...
                if (slot === 0) {
                    ...
                    if (Array.isArray(nextContents)) {
                        nextStyle = nextContents[nextContents.length -
                            1].props.className;
                    ...
                    if (Array.isArray(prevContents)) {
                        prevStyle = prevContents[prevContents.length -
                            1].props.className;
                    ...
                    if (prevStyle && prevStyle.includes(
                            "privates-right")) {
                        className += " no-border";
                    }
                }
                if (prevContents && Array.isArray(prevContents)) {
                    if (lessonQueue[1].includes(cellContents) ||
                        cellContents.includes("Private")) {
    ...
    createGridList(duration, gridArrays, lessonQueue) {
        for (let gridIndex = 0; gridIndex < gridArrays.length; gridIndex++) {
            let newTable = this.createTable(duration, gridArrays
                , gridIndex, lessonQueue);
    ...
    addModalContent(tableListElement) {
        ...
        header = header.replace(reHashNumber, "#" + (
            index + 1
        ));
        Object.assign(this.gridModal.state.header, [
            header, this.gridModal.state.header[1]
        ]);
    ...
    render() {
        return ( <
            div >
            <
            h2 className = "content-head is-right" > The Grid <
            /h2> <
            SectionDescription additionalData = {
                [React.createElement(UnorderedList, {
                    callback: (ref) => this.setComponentReference(
                        "emptyGridsNotification", ref)
                    , data: [{
                        data: [
                            "No Grids were created"
                        ]
            ...
            anchorCallback = {
                (ref) => this.setComponentReference(
                    "createGridButton", ref)
            }
            anchorHandleClick = {
                () => null
            }
            ...
            Input callback = {
                (ref) => this.setComponentReference(
                    "startTimeInputField", ref)
            }
            handleBlur = {
                this.setLessonTimes.bind(this)
            }
            placeholder = {
                "..."
            }
            ...
            UnorderedList data = {
                [{
                    data: [React.createElement(Anchor, {
                        callback: (ref) => this.setComponentReference(
                            "durationContainer"
                            , this.durationContainer
                            .concat(ref)
                    ...
                    data: [React.createElement(DurationButton
                        , {
                            callback: (ref) => this.setComponentReference(
                                "durationContainer"
                                , this.durationContainer
            ...
            /> <
            /div> <
            WaitIndicator callback = {
                (ref) => this.setComponentReference("waitIndicator"
                    , ref)
            }
            data = {
                "Creating..."
            ...
            /> <
            div className =
            "pure-menu pure-menu-horizontal pure-menu-scrollable" >
            <
            UnorderedList callback = {
                (ref) => this.setComponentReference("gridList", ref)
            }
            data = {
                []
            }
            ...
            callback = {
                (ref) => this.setComponentReference("gridModal"
                    , ref)
            }
            footer = {
                [
            ...
            header = {
                [
                    ...
                    , React.createElement(Anchor, {
                        data: "Export to PDF"
                        , handleClick: this.exportToPDF.bind(
                            this)
                        , key: "key-grid-anchor-5"
            ...
            /> <
            Tutorial buttonClass = {
                ""
            }
            callback = {
                (ref) => this.setComponentReference("tutorial", ref)
            }
            data = {

...
class Privates extends React.Component {
    ...
    getInstructorIdByName(instructorName) {
        if (instructorName in this.state.data) {
            ...
            return this.props.getInstructorData().then((res) => {
        ...
        allKeys.sort().forEach((key) => {
            ...
            for (let i = 0; i < privates.length; i++) {
                if (key in privates[i]) {
                    let sortedTimes = this.sortPrivatesByTime(
                        privates[i][key]);
    ...
    sortPrivatesByTime(privates) {
        ...
        allTimes.sort((firstEl, secondEl) => {
            ...
            if (parseInt(firstHour) === parseInt(secondHour)) {
                return parseInt(firstSeconds) < parseInt(
                    secondSeconds);
            } else {
                return parseInt(firstHour) > parseInt(
                    secondHour);
            }
        }).forEach((time) => {
            for (let i = 0; i < privates.length; i++) {
    ...
    getTableBody() {
        ...
        for (let instructorName in this.state.data) {
            ...
            for (let lessonIndex = 0; lessonIndex < instructor.length; lessonIndex++) {
                ...
                let tableRow = [privateLesson.id, instructorName
                    , privateLesson.time, privateLesson.duration
                ];
    ...
    addPrivateLesson(privateLessonBody) {
        return this.getInstructorIdByName(privateLessonBody.instructor)
            .then((res) => {
                ...
                privateLessonBody.duration = parseInt(
                    privateLessonBody.duration, 10);
                privateLessonBody.instructorId = res;
                delete privateLessonBody.instructor;
            }).then(() => this.props.createComponent(privateLessonBody
                , "Privates")).then(
                (res) => {
        ...
        for (let instructorName in privateLessons) {
            ...
            for (let lessonIndex = 0; lessonIndex < lessonArray.length; lessonIndex++) {
                ...
                if (lesson.id === instructorId) {
                    this.props.removeComponent(instructorId, "Privates")
                        .then(() => {
                            ...
                            if (lessonArray.length === 0) {
                                delete privateLessons[
                                    instructorName];
                            }
                            this.setState({
                                data: privateLessons
                            }, () => this.privatesTable.toggleState(
                                true));
                        });
                    break;
                }
    ...
    updatePrivateLesson(id, newDataRow) {
        ...
        [privateLesson, instructorName, lessonIndex] = this.findPrivateById(
            id);
        ...
        if (instructorName === newInstructorName) {
            ...
            this.getInstructorIdByName(newInstructorName).then((
                newInstructorId) => {
                if (newInstructorName in privateLessons) {
                    // Add to new instructor.
                    privateLesson.instructorId =
                        newInstructorId;
                    privateLessons[newInstructorName].push(
                        privateLesson);
                    // Remove from old instructor.
                    privateLessons[instructorName].splice(
                        lessonIndex, 1);
                    // Remove the instructor if there are no private lessons.
                    if (privateLessons[instructorName].length ===
                        0) {
                ...
                this.props.callback(this.state, "privates"
                    , true);
            }).catch((error) => console.error(error));
        }
    }
    ...
    render() {
        return ( <
            ...
            Private Lessons < /h2> <
            SectionDescription anchorCallback = {
                (ref) => this.setComponentReference("editButton"
                    , ref)
            }
            anchorHandleClick = {
                this.editPrivates.bind(this)
            }
            buttonText = {
                "Edit Privates"
            }
            ...
            title = {
                "Detail private lessons"
            }
            type = {
                "ribbon"
            }
            /> <
            PrivatesTable addCallback = {
                this.addPrivateLesson.bind(this)
            }
            callback = {
                (ref) => this.setComponentReference("privatesTable"
                    , ref)
            }
            dataBody = {
                this.getTableBody()
            ...
            removeCallback = {
                this.removePrivateLesson.bind(this)
            }
            updateCallback = {
                this.updatePrivateLesson.bind(this)
            }
            /> <

...
class GridChecklist extends React.Component {
    ...
    setQuantity(key, quantity) {
        Object.assign(this.state.data[key], {
            quantity: quantity
        });
        this.checkComplete();
    ...
    checkComplete() {
        ...
        for (let key in this.state.data) {
            ...
            if (key === "instructors") {
                ...
                let errorClass = privatesQuantity > 0 ?
                    "warning-cell" :
                    "error-cell";
                Object.assign(value, {
                    cell: isValid ?
    ...
    getCellStyle(value) {
        ...
        if (this.componentRow === "Instructors") {
            ...
            let errorClass = this.state.data.privates.quantity > 0 ?
                "warning-cell" :
                "error-cell";
            cellClass = isValid ?
                "" :
    ...
    render() {
        return ( <
            ...
            Table dataBody = {
                [
                    [
                        "Instructors", this.state.data.instructors.quantity
                        .toString()
                    ]
                    , [
                        "Lessons", this.state.data.lessons.quantity
                        .toString()
                    ]
                    , [
                        "Privates", this.state.data.privates.quantity
                        .toString()
                    ]
                ]
            }

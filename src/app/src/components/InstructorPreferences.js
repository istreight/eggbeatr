...
class InstructorPreferences extends React.Component {
        constructor(props) {
            ...
            this.defaultPreferences = [
                [
                    "Starfish", "Sea Otter", "Level 1", "Level 6"
                    , "Basics I"
                ]
                , [
                    "Duck", "Salamander", "Level 2", "Level 7"
                    , "Basics II"
                ]
                , [
                    "Sea Turtle", "Sunfish", "Level 3", "Level 8"
                    , "Strokes"
                ]
                , [
                ...
                , [
                    "Simple Set", "Whale", "Level 5", "Level 10"
                    , "Schoolboard"
                ]
            ];
        }
        componentDidMount() {
            var dynamicInstructorPreferences = document.getElementById(
                "dynamicInstructorPreferences"
            );
            ...
            window.addEventListener("click", (e) => {
                if (e.target === ReactDOM.findDOMNode(this.modal)) {
                    this.modal.setState({
                        ...
                        "preferences") ||
                    dynamicInstructorPreferences.contains(e.target)
                ) {
                    this.modal.setState({
                        isDisplayed: true
                    });
        ...
        updatePreferenceCell(e) {
            ...
            var instructor = this.selectedInstructor;
            preferences = JSON.parse(JSON.stringify(this.state.data[
                instructor]));
            ...
            for (let rowIndex = 0; rowIndex < preferences.length; rowIndex++) {
                ...
                for (let colIndex = 0; colIndex < row.length; colIndex++) {
                    ...
                    preferences[rowIndex][colIndex] = [React.createElement(
                        Anchor, {
                            data: lessonType
                            , handleClick: this.updatePreferenceCell
                                .bind(this)
                            , key: "key-anchor-" + (
                                rowIndex * preferences.length +
                                colIndex
                            )
                            , styleClass: ""
                        })];
        ...
        displayComponentState(instructorName) {
            ...
            if (instructorName in this.state.data) {
                ...
                this.props.getPreferenceData().then((res) => {
                    this.setState(
                        res
                        , () => this.modal.setState({
                            header: [instructorName]
        ...
        finishEditing() {
            this.editButton.setState({
                ...
                , () => this.setState(this.state, () => this.props.callback(
                    this.state, "instructorPreferences", true))
            );
        }
        /**
                ...
                return ( <
                    Modal body = {
                        [ < p key = "key-modal-p-0" >
                            ...
                            #39;s level preferences.
                </p>
                    ]}
                callback= {
                                (ref) => this.setComponentReference("modal"
                                    , ref)
                            }
                            footer = {
                            ...
                            tableData = {
                                {
                                    ...
                                    , styleCell: this.getCellStyle.bind(
                                        this)
                                    , styleRow: (index) => (
                                        index % 2 ?
                                        "table-even" :

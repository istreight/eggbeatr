...
class InstructorTable extends React.Component {
    ...
    getAdditionalRowData(instructor, index) {
        ...
        checkboxObject = this.getPrivatesOnlyCheckbox(instructor[4]
            , index);
        prefsButtonObject = this.getPreferencesButton(instructor[5]
            , index);
    ...
    getPreferencesButton(prefConfig, keyIndex) {
        return [React.createElement(PreferencesButton, {
            callback: (ref) => {
                this.preferencesButtons = this.preferencesButtons
                    .filter(
                        (checkbox) => ref.state.instructorId !==
                        checkbox.state.instructorId
                    );
    ...
    getPrivatesOnlyCheckbox(privatesOnlyConfig, keyIndex) {
        return [React.createElement(PrivatesOnlyCheckbox, {
            callback: (ref) => {
                this.privatesOnlyCheckboxes = this.privatesOnlyCheckboxes
                    .filter(
                        (checkbox) => ref.state.instructorId !==
                        checkbox.state.instructorId
                    );
    ...
    remove(instructorId) {
        ...
        this.privatesOnlyCheckboxes.forEach((privatesCheckbox
            , privIndex) => {
            if (privatesCheckbox.state.instructorId ===
                instructorId) {
    ...
    toggleState(enable) {
        for (
            let rowIndex = 0; rowIndex < this.privatesOnlyCheckboxes.length; rowIndex++
        ) {
    ...
    render() {
        return ( <
            ...
            customStyle = {
                this.checkWSIExpiration.bind(this)
            }
            dataBody = {
                this.state.dataBody
            }
            ...
            getAdditionalRowData = {
                this.getAdditionalRowData.bind(this)
            }
            inputValueValidations = {
                this.getInputValueValidations()
            }
            removeCallback = {

...
class ExportToPDF extends React.Component {
    ...
    pdf(setTitle, lessonTimes) {
        ...
        columns = [{
            ...
            dataKey: "startPlusHalf"
            , title: lessonTimes[1]
        }, {
            dataKey: "startPlusOne"
            , title: lessonTimes[2]
        }, {
            dataKey: "startPlusOneAndHalf"
            , title: lessonTimes[3]
        }, {
            dataKey: "startPlusTwo"
            , title: lessonTimes[4]
        }];
        ...
        tableRows.each((rowIndex, element) => {
            ...
            $(element).find("td").each((cellIndex, element) => {
                ...
                if (cellIndex === 0 && reName.test(
                        cellText)) {
                    instructors.push(cellText);
                }
                rowsKey = columns[cellIndex].dataKey;
                if (threeQuarterLessons.includes(
                        cellText)) {
                    let index = rowIndex * numColumns +
                        cellIndex;
                    if ($(element).prev().children().length >
                        0) {
                        splitCellIndices.push(index - 1);
                    }
                    splitCellIndices.push(index);
                    if ($(element).next().children().length >
                        0) {
                        splitCellIndices.push(index + 1);
                    }
                }
        ...
        doc.autoTable(columns, rows, {
            ...
            , startY: 60
            , addPageContent: (data) => {
                tableCoordinates = this.addPageContent(doc
                    , data, setTitle);
            }
            , createdCell: (cell, data) => {
                ...
                cell = this.createdCell(cell, data
                    , isRowOdd, numColumns
                    , splitCellIndices);
            }
            , drawCell: (cell, data) => {
    ...
    addPageContent(doc, data, setTitle) {
        ...
        return [
            data.table.pageStartX, data.table.pageStartY, data.table
            .width + data.table.pageStartX
    ...
    drawCell(
        ...
        if (splitCellIndices.includes(cellIndex) && prevCell) {
            if (threeQuarterLessons.includes(prevCell.text[0]) &&
                quarterActivities.includes(cell.text[0])) {
            ...
            } else if (threeQuarterLessons.includes(cell.text[0]) &&
                quarterActivities.includes(prevCell.text[0])) {
                lineCoordinates.push([
    ...
    drawLines(doc, lineCoordinates, tableCoordinates) {
        ...
        // Draw table border lines (to compensate for removing cell borders on edge of
        // table).
